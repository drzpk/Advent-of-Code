...
class SolutionNotFoundException(BaseException):
    def __init__(self, year, day, part):
        super(self, f"Solution not found for {year}-{day}, part {part}")
...
def reduce_snailfish_number(snailfish_number: str) -> str:
    while True:
        ...
        if exploded:
            print("after explode:  " + snailfish_number)
            continue

        snailfish_number, was_split = split(snailfish_number)
        if was_split:
            print("after split:    " + snailfish_number)
...
def parse_snailfish_number(snailfish_number: str, next_value: str) -> str:
    ...
    print("after addition: " + snailfish_number)
    return reduce_snailfish_number(snailfish_number)


def parse_snailfish_numbers(snailfish_numbers: Union[list[str], tuple[str, ...]]) -> str:
...
def get_input_for_day(year, day):
    path = f"main/resources/{year}/day{day}/example.txt"
    with open(path, "r") as h:
        return [x.strip() for x in h.readlines()]


if __name__ == '__main__':
    data = get_input_for_day(2021, 18)
    print("part1: " + str(part_one(data)))
    # part_two(data)
